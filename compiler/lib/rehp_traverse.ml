...
class type mapper =
  object
    ...
    method initialiser_o :
      (Rehp.expression * Loc.t) option -> (Rehp.expression * Loc.t) option
    ...
    method statement_o :
      (Rehp.statement * Loc.t) option -> (Rehp.statement * Loc.t) option
...
class share_constant =
  object
    ...
    method program p =
      ...
      let shareit_num (prefix, s) =
        let l = String.length s in
        if l > 2 then Some (prefix ^ s) else None
      in
      Hashtbl.iter
        (fun x n ->
          let shareit =
  ...
  object (m)
    ...
    method expression e =
      ...
      | EBin (Plus, e1, e2) -> (
        ...
        | EInt n, _ when n < 0 -> EBin (Minus, e1, EInt (-n))
        | _, EFloat n when n < 0. -> EBin (Minus, e2, EFloat (-.n))
        | _, EInt n when n < 0 -> EBin (Minus, e2, EInt (-n))
      ...
      | EBin (Minus, e1, e2) -> (
        ...
        | EInt n, _ when n < 0 -> EBin (Plus, e1, EInt (-n))
        | (EFloat _ as x), EFloat 0. -> x
        | (EInt _ as x), EInt 0 -> x
        | _ -> e)
      | _ -> e
